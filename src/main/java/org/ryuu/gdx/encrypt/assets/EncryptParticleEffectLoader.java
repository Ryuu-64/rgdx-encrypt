...
import org.ryuu.gdx.encrypt.file.ChildNameEncryptFileHandle;
...
public class EncryptParticleEffectLoader extends SynchronousAssetLoader<ParticleEffect, EncryptParticleEffectLoader.ParticleEffectParameter> {
    public EncryptParticleEffectLoader(FileHandleResolver resolver) {
        super(resolver);
    }

    @Override
    public ParticleEffect load(AssetManager assetManager, String fileName, FileHandle file, ParticleEffectParameter param) {
        ParticleEffect effect = new ParticleEffect();
        if (param != null && param.atlasFile != null) {
            effect.load(file, assetManager.get(param.atlasFile, TextureAtlas.class), param.atlasPrefix);
        } else if (param != null && param.imagesDir != null) {
            effect.load(file, param.imagesDir);
        } else {
            effect.load(file, new ChildNameEncryptFileHandle(file.parent()));
        }
        return effect;
    }
    ...
    public Array<AssetDescriptor> getDependencies(String fileName, FileHandle file, ParticleEffectParameter param) {
        Array<AssetDescriptor> assetDescriptors = null;
        if (param != null && param.atlasFile != null) {
            assetDescriptors = new Array<>();
            assetDescriptors.add(new AssetDescriptor<>(param.atlasFile, TextureAtlas.class));
        }
